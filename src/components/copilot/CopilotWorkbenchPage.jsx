...
import {
  ...
  streamCopilotChat,
} from '../../services/copilotChatApi.js';
import {
...
import {
  getCopilotProjectUserId,
  getCopilotServiceHealth,
} from '../../services/copilotServiceApi.js';
import {
...
export default function CopilotWorkbenchPage({
  ...
  const [currentModel, setCurrentModel] = useState('EdgeOne');
  ...
  useEffect(() => {
    ...
    getCopilotServiceHealth({ signal: controller.signal })
      .then((health) => setCurrentModel(health?.model || 'EdgeOne'))
      .catch(() => setCurrentModel('EdgeOne'));
    return () => controller.abort();
  }, []);

  useEffect(() => {
    // 切换项目时中断当前流式任务并重载项目会话缓存。
    for (const conversationId of activeChatControllersRef.current.keys()) {
  ...
  function deleteConversation(conversationId) {
    ...
    void deleteCopilotConversation(
      conversationId,
      getCopilotProjectUserId(activeProject.id),
    ).catch(() => {});
  ...
  async function submitArticlePlanningForm(messageId, block, values) {
    // 确认表单后沿用当前 waiting_input 运行继续工作流，不依赖输入框中的自然语言。
    if (block.submitted || activeConversationRunning) return;
    updateMessageUiBlock(messageId, block.id, { submitted: true, values });
    await submitChat(null, {
      composerDraft: {
        knowledgeFileIds: block.knowledgeFileIds ?? [],
        knowledgeItemIds: block.knowledgeItemIds ?? [],
        text: '',
      },
      content: '已确认文章策划信息。',
      planningInputs: values,
      planningInputsConfirmed: true,
    });
  }
  ...
  function upsertArticlePlanningForm(messageId, eventData) {
    // waiting_input 中的 inputForm 映射为现有文章任务表单，并随消息持久化。
    if (eventData.inputForm?.formType !== 'article_planning') return;
    const blockId = `article-task-form-${eventData.taskId}`;
    patchConversationState((currentState) => ({
      ...currentState,
      messages: currentState.messages.map((message) => {
        if (message.id !== messageId) return message;
        ...
        const nextBlock = {
          ...currentBlock,
          agentId: eventData.agentId || 'seo_strategist',
          id: blockId,
          knowledgeFileIds: currentBlock?.knowledgeFileIds ?? [],
          knowledgeItemIds: currentBlock?.knowledgeItemIds ?? [],
          missingFields: eventData.inputForm.missingFields ?? [],
          submitLabel: eventData.inputForm.submitLabel,
          submitted: false,
          type: 'article_task_form',
          uncertainFields: eventData.inputForm.uncertainFields ?? [],
          values: currentBlock?.values ?? eventData.inputForm.values ?? {},
        };
        if (blockIndex >= 0) blocks[blockIndex] = nextBlock;
        else blocks.push(nextBlock);
        return { ...message, uiBlocks: blocks, updatedAt: new Date().toISOString() };
  ...
  async function submitChat(event, options = {}) {
    // 提交聊天消息后创建用户消息、助手占位消息和运行记录。
    event?.preventDefault?.();

    const content = String(options.content ?? chatInput).trim();
    if (!content) return;
    ...
    const composerDraft = options.composerDraft ?? composerDrafts[currentConversation.id] ?? emptyComposerDraft;
    ...
    try {
      await streamCopilotChat({
        body: {
          ...
          planningInputsConfirmed: Boolean(options.planningInputsConfirmed),
          projectId: activeProject.id,
          taskContinuation,
          targetArtifact,
          userId: getCopilotProjectUserId(activeProject.id),
        },
        conversationId: currentConversation.id,
        ...
        onEvent: (eventData) => {
          ...
          if (eventData.type === 'task_status') {
            ...
            if (eventData.status === 'waiting_input' && eventData.inputForm) {
              upsertArticlePlanningForm(assistantMessage.id, eventData);
            }
                ...
                <div className="mt-3">
                  {block.type === 'article_task_form' ? (
                    <ArticleTaskForm
                      ...
                      onSubmit={(values) => submitArticlePlanningForm(message.id, block, values)}
                    />
  ...
  function getChatErrorMessage(error) {
    ...
    if (
      ...
      /Failed to fetch|No SSE events|Copilot service/i.test(
        message,
      )
    ) {
