...
const edgeOneConversationResetVersion = 1;
...
// EdgeOne 迁移后删除旧版单键会话缓存，避免遗留历史继续占用浏览器存储。
function removeLegacyConversations(projectId) {
  if (typeof window === 'undefined') return;
  window.localStorage.removeItem(`${legacyConversationsStorageKeyPrefix}${projectId}`);
}
...
export function getCopilotConversationState(projectId) {
  ...
  const shouldResetForEdgeOne =
    seedMeta.edgeOneConversationResetVersion !== edgeOneConversationResetVersion;
  const shouldCleanupDefaultHistory =
    !shouldResetForEdgeOne &&
    seedMeta.defaultHistoryCleanupVersion !== defaultHistoryCleanupVersion;
  const shouldInstallDemoSeed =
    !shouldResetForEdgeOne &&
    Boolean(demoSeed) &&
    seedMeta.copilotDemoSeedVersion !== copilotDemoSeedVersion;

  if (shouldResetForEdgeOne) {
    removeLegacyConversations(projectId);
    state = {
      artifacts: [],
  ...
  if (shouldResetForEdgeOne || shouldCleanupDefaultHistory || shouldInstallDemoSeed) {
    writeProjectTable(projectId, tableNames.seedMeta, {
      ...seedMeta,
      copilotDemoSeedVersion: shouldResetForEdgeOne || shouldInstallDemoSeed
        ? copilotDemoSeedVersion
        : seedMeta.copilotDemoSeedVersion,
      defaultHistoryCleanupVersion,
      edgeOneConversationResetVersion,
    });
  }
