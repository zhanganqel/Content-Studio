...
不要直接打开根目录的 `index.html`。这是 Vite 应用入口文件，必须通过本地服务访问；直接用文件路径打开通常会显示空白页。
...
组件规范见 `docs/component-guidelines.md`。`package.json` 中保留了 `guidelines:*` 脚本，但当前仓库未包含独立 `guidelines/` 目录，研发接手时以 `docs/component-guidelines.md` 为准。
...
| 任务 | 修改入口 |
| --- | --- |
| 新增导航项或模块 | `src/data/navigation.js`、`src/i18n/messages.js`、`src/components/MainContent.jsx` |
| 新增业务页面 | 在 `src/components/` 下建页面目录，并接入 `MainContent.jsx` |
| 新增系统文案 | `src/i18n/messages.js`，必须同时补齐 `zh-CN` 和 `en-US` |
| 新增演示项目数据 | 先写入 `demo-data/rejin-cnc/`，再同步到 `src/data/demo/` |
| 新增本地保存逻辑 | `src/services/` |
| 新增图片素材 | `demo-data/rejin-cnc/assets/images/`、`assets/image-library.md`、`rejinCncAssetLibrary.js` |
| 新增资料文件 | `demo-data/rejin-cnc/file/`、`file-library.md`、`rejinCncFileLibrary.js` |
| 调整按钮、页面头部、Toast | `src/components/ui/` 与 `docs/component-guidelines.md` |
| 调整博客 AI 创作流程 | `src/components/blog-article/` 与 `src/services/blogArticleAiStore.js` |
